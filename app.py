...
def kirim_absen_silent(nama, is_pns):
    base_form = "1FAIpQLSdfwUrcxoTer6M2NEMOpxoFYF8e9lBe5reG7rF1ZQIdtjRwzA" if is_pns else "1FAIpQLSe4pgHjDzZB9OTgbq7XNw5SWTNIo0AjTnnVUukd13e9BgkNPw"
    post_url = f"https://docs.google.com/forms/d/e/{base_form}/formResponse"
    ...
    try:
        # Kirim tanpa perlu nunggu balasan konten yang berat
        r = requests.post(post_url, data=payload, headers=headers, timeout=10, allow_redirects=True)
        # Apapun yang dikembalikan Google (200, 302, 403), kita anggap saja masuk jika koneksi OK
            ...
            if st.button(f"👤 {nama_tombol}", key=f"btn_{idx}_{n}", use_container_width=True):
                with st.spinner('Proses...'):
                    if kirim_absen_silent(n, n in MASTER_PNS):
                        st.toast(f"✅ Terkirim ke Google: {nama_tombol}", icon="🚀")
                        st.cache_data.clear()
                        time.sleep(2)
                        st.rerun()
