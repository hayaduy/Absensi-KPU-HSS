...
def fetch_data(url):
    try:
        ...
        # PENGAMAN 1: Paksa konversi datetime, abaikan yang error
        df.iloc[:, 0] = pd.to_datetime(df.iloc[:, 0], dayfirst=True, errors='coerce')
        # PENGAMAN 2: Buang baris yang gagal konversi (NaT)
...
def process_log(df, tgl):
    ...
    if not df.empty:
        try:
            ...
            # PENGAMAN 3: Filter hanya baris yang punya tipe data datetime
